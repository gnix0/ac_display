...
The current implementation uses C's math lib for simulating true AC readings, although the code for the signal readings is implemented and validated (although commented). Since neither the S3 or the C6 have a DAC, the last step of the project would be to configure and use an external one
to display the true reading values, which will be done when I have the time. Any changes in the meantime will focus only on making the rendering smoother and more optimal.
